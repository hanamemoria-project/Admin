...
function updateJam() {
  ...
  document.getElementById('jam-sidebar').textContent =
...
function showSection(s, el) {
  // 'orders' maps to dashboard section
  const sectionId = (s === 'orders') ? 'dashboard' : s;

  // Hide all sections
  ['dashboard', 'keuangan'].forEach(name => {
...
// Init: show dashboard, hide keuangan
...
function renderTabel() {
  ...
  const produk  = document.getElementById('filter-produk').value;
  ...
  let filtered = dataPesanan.map(p => {
    ...
    return { ...p, _status: s, _mindar: mindar };
...
function bukaDetail(id) {
  ...
  // Show QR preview with consistent URL
...
async function downloadQRCode() {
  ...
  const H           = 920;           // canvas height — lebih tinggi agar teks bawah tidak tertumpuk

  const outerPad    = 36;            // jarak card dari tepi canvas

  const scanMeH     = 72;            // tinggi area SCAN ME di atas

  const bottomH     = 96;            // tinggi area teks Order ID + brand di bawah

  const innerGap    = 18;            // gap antara border luar & dalam

  const qrMargin    = 16;            // gap antara border dalam & QR
  ...
  try {
    ...
    // ── Inner solid brown border ──

    // Top of inner border starts below SCAN ME area
    ...
    // ── SCAN ME label (bigger, centered at top) ──
    ...
    const scanCY = cardY + scanMeH / 2 + 2;  // vertical center of SCAN ME zone
...
async function jalankanCetak(orientasi) {
  ...
  for (const cb of cbTerpilih) {
    ...
    // Use consistent QR URL — same as modal and download
...
const KATEGORI_MASUK = ['penjualan','deposit','refund_masuk','lain-lain'];
const KATEGORI_KELUAR = ['bahan baku','ongkos kirim','operasional','marketing','lain-lain'];
...
function buatGrafikProduk() {
  ...
  // Soft elegant palette: rose gold, sage, sky
    ...
    grafikProduk = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          ...
          hoverBackgroundColor: softHover,
          borderRadius: 8,
          borderSkipped: false
        }]
      },
...
window.onload = async () => {
  ...
  if (!sudahLogin) {
    // Tampilkan login screen, sembunyikan loading
    if (typeof window.__hideInstantLoading === 'function') window.__hideInstantLoading();
    hideLoading();
